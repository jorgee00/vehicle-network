...
let NewSystemRequestContract = NewSystemRequestContract_1 = class NewSystemRequestContract extends fabric_contract_api_1.Contract {
    ...
    */ 
    ...
    async sendNewSystemDescription(ctx, nombre, sw_included) {
        ...
        const trade = new system_request_1.Sistema();
        trade.nombre = nombre;
        trade.sw_included = sw_included;
        const buffer = Buffer.from(JSON.stringify(trade));
        await ctx.stub.putState(nombre, buffer);
    }
    /*@Transaction()
    public async acceptVehicle(ctx: Context, tradeId: string, justification: string): Promise<void> {
    ...
    @Transaction()
    public async rejectVehicle(ctx: Context, tradeId: string, justification: string): Promise<void> {
        const trade = await this.getVehicleDescription(ctx, tradeId);
...
__decorate([
    ...
    __metadata("design:paramtypes", [fabric_contract_api_1.Context, String]),
    __metadata("design:returntype", Promise)
], NewSystemRequestContract.prototype, "getVehicleDescription", null);
