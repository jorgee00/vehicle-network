...
//import { AvisoActualizacion } from '.';
...
const oemManufQuery = {
    selector: {
        exporterMSP: 'OEMManufacturerOrg',
    },
    use_index: ['_design/oemManufIndexDoc', 'oemManufIndex'],
};
const homologatorQuery = {
    selector: {
        importerMSP: 'HomologatorOrg',
    },
    use_index: ['_design/homologatorIndexDoc', 'homologatorIndex'],
};
...
class TestHistoryQueryIterator {
    constructor(resultset) {
        this.index = 0;
        this.resultset = resultset;
    }
...
describe('As an importer, I can enter in a trade agreement with an exporter to acquire specific goods', () => {
    let contract;
    let ctx;
    async function beforeTestList(exporterObjList, importerObjList, regulatorObjList, clientMSP) {
        let exporterResultset = [];
        if (exporterObjList !== null) {
            exporterObjList.forEach((exporterObj) => {
                const exporterJsonStr = JSON.stringify(exporterObj);
        ...
        if (importerObjList !== null) {
            importerObjList.forEach((importerObj) => {
                const importerJsonStr = JSON.stringify(importerObj);
        ...
        if (regulatorObjList !== null) {
            regulatorObjList.forEach((regulatorObj) => {
                const regulatorJsonStr = JSON.stringify(regulatorObj);
                regulatorResultset = regulatorResultset.concat({ namespace: 'somenamespace', key: 'somekey', value: Buffer.from(regulatorJsonStr) });
            });
        }
        const exporterIterator = new TestStateQueryIterator(exporterResultset);
        const importerIterator = new TestStateQueryIterator(importerResultset);
        const regulatorIterator = new TestStateQueryIterator(regulatorResultset);
...
//# sourceMappingURL=new_software_request-contract.spec.js.map